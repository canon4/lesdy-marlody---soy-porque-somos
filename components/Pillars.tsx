...
import { BookOpen, Map, Feather, TrendingUp, ShieldCheck, Gavel, Quote } from 'lucide-react';
...
const Pillars: React.FC = () => {
  ...
  const getColorClasses = (color: string) => {
    switch(color) {
      case 'jungle': return 'bg-jungle/10 text-jungle border-jungle';
      case 'success': return 'bg-success/10 text-success border-success';
      case 'gold': return 'bg-gold/10 text-gold-dark border-gold';
      case 'earth': return 'bg-earth/10 text-earth border-earth';
        ...
        <div className="text-center mb-16">
          ...
          <div className="max-w-3xl mx-auto bg-white p-6 rounded-xl shadow-sm border border-gray-100">
            ...
            <p className="text-textmuted text-lg">
              Cada compromiso está pensado desde la <strong className="text-jungle">incidencia legislativa</strong>, 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-20">
          {pillars.map((pillar, idx) => {
            ...
            return (
              ...
              >
                <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-6 ${colorClass} border-2`}>
                  {pillar.icon}
                </div>
                ...
                <ul className="space-y-4">
                  {pillar.items.map((item, i) => (
                    <li key={i} className="flex items-start gap-3">
                      <span className={`w-2 h-2 rounded-full mt-2 shrink-0 ${pillar.color === 'jungle' ? 'bg-jungle' : pillar.color === 'success' ? 'bg-success' : pillar.color === 'earth' ? 'bg-earth' : 'bg-gold'}`}></span>
